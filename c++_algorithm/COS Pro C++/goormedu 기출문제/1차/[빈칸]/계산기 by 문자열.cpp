...
int solution(string expression) {
    int exp_index = func_b(expression); // 이 부분
    Pair numbers = func_c(expression, exp_index); // 이 부분
    int result = func_a(numbers.first_num, numbers.second_num, expression[exp_index]); // 이 부분
    return result;
}
